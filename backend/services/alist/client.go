...
import (
	...
	"astrm/backend/utils/concurrent"
	"astrm/backend/utils/iterator"
	"astrm/backend/utils/pandora"
	"context"
	"encoding/json"
	"fmt"
	"io"
...
func (a *Client) Handle(j *models.Task) (err error) {
	...
	process := func(content *Content, o *models.SaveOpt) {
		...
		case 1:
			...
			if err != nil {
				log.Printf("alist Stream 失败: %v", err)
				return
			}
			o.Body = result.Body
		...
		default:
			...
			case "raw_url":
				...
				if get, err := a.FsGet(ctx, content.Name); err == nil {
					...
					log.Printf("获取 alist raw_url 失败: %v", err)
					return
				}
		...
		err = Save(*o)
		if err != nil {
			log.Printf("获取 alist Save 失败: %v", err)
			return
		}
	...
	for _, from := range strings.Split(j.From, "\n") {
		...
		for ct := range it.Iter() {
			if ct.Error != nil {
				err = ct.Error
				log.Printf("alist FsList 失败: %v", err)
				continue
			}
...
func (a *Client) Json(ctx context.Context, uri, method, data string, headers map[string]any) (result Result, err error) {
...
func (a *Client) Stream(ctx context.Context, uri, method, data string, headers map[string]any) (res *http.Response, err error) {
...
func (a *Client) FsList(ctx context.Context, path string, recursion bool, opts *models.TaskOptions) (res *iterator.Iterator[*Content]) {
...
func (a *Client) FsGet(ctx context.Context, path string) (content FsGet, err error) {
	var result Result
...
func Save(opt models.SaveOpt) (err error) {
	...
	log.Printf("[Save] %s -> %s ", opt.From, filePath)
	return
